...
class Depth_estimation_decoder(nn.Module):
    ...
    def forward(self, F_I):
        ...
        for i in range(4, -1, -1):
            #self.compr[i]用来将F_I的通道全部调整为64
            #其实这里的u_decoder就是类似UNet的解码，传进去的就是Fi和feat
            #self.compr就是为了将F_I中的几个feateure缩放到channel=64
            F_I_ = self.compr[i](F_I[i])
            feat, estimated_depth = self.u_decoder[i](F_I_, feat)
        #归一化
...
class High_level_fusion(nn.Module):
    ...
    def forward(self, F_I, F_fuse, vgg_rgb, compr_I):
        ...
        #进行高层语义特征融合，第4层和第5层特征图
        F_fuse_4, F_fuse_5 = F_fuse[3], F_fuse[4]
        f_d_4, P_init = self.FAM_1([F_fuse_4, F_fuse_5], 28) #空间对齐
...
class Net(nn.Module):
    ...
    def forward(self, img):
        ...
        I = img
        #对应于原paper的RGB Encoder部分
        #F_I存放着F1，F2，F3，F4，F5阶段的RGB特征
        ...
        #先将原始深度图的channel赋值为和RGB一样：3，
        # 然后用和RGB Encoder一样的编码器提取特征
